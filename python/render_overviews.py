"""MechProof — scene preview renderer (WebGPU / pygfx).

Reads a MuJoCo MJCF for *geometry and pose information only* — never
touches `mujoco.Renderer` — and rebuilds the scene as a `pygfx` scene
graph that renders via WebGPU. Output is one PNG per scene per camera
angle, written to `out/preview_<scene>_<angle>.png`.

Why WebGPU rather than MuJoCo's built-in OpenGL renderer?

  * MuJoCo's renderer needs an OpenGL backend (EGL / OSMesa / GLFW).
    Each of those has its own driver-detection headache on cloud CI
    runners — no EGL device on GitHub Actions, OSMesa needs libGL
    anyway, GLFW needs an X server, …
  * WebGPU has well-defined offscreen semantics. On Linux it routes
    ...
    `mesa` (both in `shell.nix`). The same code runs on macOS (Metal)
    and Windows (D3D12) too.
